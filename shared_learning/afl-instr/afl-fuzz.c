...
static struct queue_entry *queue,     /* Fuzzing queue (linked list)      */
                          *queue_temp, /* Holds all temp files for the -B option */
...
/* Write fuzzing information to allow for better testing.
   Only called at the end of the fuzzing process */

EXP_ST void write_fuzzing_info(void) {

  u8* fname;
  s32 fd;


  fname = alloc_printf("%s/fuzz_process_info.txt", out_dir);
  fd = open(fname, O_WRONLY | O_CREAT | O_TRUNC, 0600);

  if (fd < 0) PFATAL("Unable to open '%s'", fname);

  //ck_write(fd, virgin_bits, MAP_SIZE, fname);
  //ck_write(fd, out_dir, sizeof(out_dir), fname); // todo -> probably can't do this like this
  //printf("out_dir is = %s/%s\n", cwd, out_dir);
  char outdir_path[1000];

  // we just write the path to the outdir // is this even needed?
  sprintf( outdir_path, "%s/%s\n", cwd, out_dir);

  //printf("%s\n", outdir_path);
  u8 size;
  for( size = 0; outdir_path[size] != '\0'; size++ );
  
  ck_write(fd, outdir_path, size, fname); // todo -> probably can't do this like this
...
/* Obtain information from a previous fuzzing process */

EXP_ST void read_fuzzing_knowledge(u8* fname) {
  ...
  //ck_read(fd, virgin_bits, MAP_SIZE, fname);
  // TODO -> obtain first line with path to previous outdir
  // TODO-> obtain second line with path to bitmap
...
static void perform_dry_run_on_temp(char** argv) {

  struct queue_entry* q = queue_temp;
  u32 cal_failures = 0;
  u8* skip_crashes = getenv("AFL_SKIP_CRASHES");
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault, u8 shw_sts) {
  ...
  if (fault == crash_mode) {
    ...
    //res = calibrate_case(argv, queue_top, mem, queue_cycle - 1, 0, shw_sts);
...
static u8 fuzz_one(char** argv) {
  ...
  if (queue_cur->cal_failed) {
    ...
    if (queue_cur->cal_failed < CAL_CHANCES) {
      ...
      res = calibrate_case(argv, queue_cur, in_buf, queue_cycle - 1, 0, 1);
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:Q")) > 0)
    ...
    switch (opt) {
      ...
      case 'B': /* load bitmap */
        ...
        //printf("-B option\n");
          if ( has_information_to_learn ) FATAL("Multiple -B options not supported");

          has_information_to_learn = 1;
          path_to_past_fuzz_info = optarg;
          // read all virgin bits in bitmap
          u8 path_to_binutils[1000];
          sprintf(path_to_binutils, "%s/%s", path_to_past_fuzz_info, "fuzz_bitmap");
          read_bitmap(path_to_binutils); 
  ...
  // TODO -> pass through the crashes/hangs/queue of the old one to obtain all information

  cull_queue();

  // path_to_past_fuzz_info

  // LEARN FROM PREVIOUS FUZZING EFFORTS
  
  if( has_information_to_learn ){
    char tmp[500];
    sprintf(tmp, "%s/%s", path_to_past_fuzz_info, "queue");

    //done for queue
    read_prev_outdir(tmp, use_argv);

    sprintf(tmp, "%s/%s", path_to_past_fuzz_info, "crashes");
    read_prev_outdir(tmp, use_argv);
    //printf("tmp = %s\n", tmp);
